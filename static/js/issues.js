document.querySelector('#TI').addEventListener('click', async (event)=> {
  const theadv = ['issue ID', 'Type', 'Priority', 'Status', 'Assigned To']
  ...
  table.append(tbody)
  let tdata = await fetch('http://localhost:8000/lprojects', {headers: {'Accept':'Application/json'}})
  ...
  for (let entry of data){
    ...
    td5.textContent = entry.AssignedTo
    tr.appendChild(td5)
